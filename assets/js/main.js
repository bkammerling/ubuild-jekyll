  ...
  init: function (target, config) {
    ...
    $(target).on(
      ...
      function (event, slick, currentSlide, nextSlide) {
        $(".picture-slider__menu-image:nth-child(" + (nextSlide + 1) + ")")
          .addClass("current")
          .siblings()
          .removeClass("current");
      }
...
var form = {
  init: function () {
    ...
    $("input, textarea, select").focus(function () {
      if (!recaptchaLoaded) {
        ...
        script.src =
          "https://www.google.com/recaptcha/api.js?render=6LeHSagUAAAAACPB5JfFS9ihSEbW-PJHqbBjlDgR";
  ...
  submit: function (contactForm) {
    ...
    if (
      $(
        ".summit #modal-form #subject-field, .autotech-events #modal-form #subject-field"
      ).length
    ) {
      var subject = "WMS";
    ...
    $.ajax({
      ...
      .fail(function (error) {
        ...
        contactForm
          ...
          .removeClass("hidden")
          .text(
            "There was a problem sending your message, please try again or ping us an email at marketing@wundermobility.com."
          );
        $("#form-submit").attr("disabled", false);
      });
  },
  ...
  serializeObject: function ($form) {
    ...
    var hidden = $("#main-contact").find(
      ".form-group:hidden select:not(#subject-field), .form-group:hidden input"
    );
  ...
  customValidityChecks: function ($form) {
    ...
    if ($form.find(".select2:visible:not(.selected)").length > 0) {
      ...
      $form
        .find(".select2:visible:not(.selected)")
        .popover({
...
function formSubmit(e) {
  ...
  if (form.htmlValidityCheck($form) && form.customValidityChecks($form)) {
    ...
    grecaptcha.ready(function () {
      grecaptcha
        .execute("6LeHSagUAAAAACPB5JfFS9ihSEbW-PJHqbBjlDgR", {
          action: "submission",
        })
        .then(function (token) {
...
var accordion = {
  init: function () {
    ...
    $(".horizontal-accordion ion-icon, .horizontal-accordion .card").click(
      function (e) {
...
var maxItems = {
  init: function () {
    ...
    $(".accordion-content").after(
      "<a class='text-center accordion-title max-showmore'>Show more</a><hr class='light'>"
    );
    accordion.init();
  },
};
...
var contentDropdown = {
  init: function () {
    $(
      ".content-dropdown .content-dropdown__opener a, .content-dropdown .content-dropdown__link"
    ).click(function (e) {
      $(e.target)
        .parents(".content-dropdown")
        .toggleClass("active")
        .find(".content-dropdown__content")
        .slideToggle();
    });
  },
};
...
if ($("input[type='range'].wunder-range").length) {
  $("input[type='range'].wunder-range").on("input", function () {
    ...
    $(".range-label")
      .css("left", "calc(" + percent + "% - 30px)")
      .text(numberWithCommas($(this).val()));
    var co2saved = $(this).val() * 0.375 * 1.553;
    $(".co2-saved h3").text(numberWithCommas(Math.round(co2saved)));
    $(".trees-planted h3").text(
      numberWithCommas(Math.round(co2saved / 0.0218))
    );
    $(".parking-saved h3").text(
      numberWithCommas(Math.round($(this).val() * 0.375))
    );
  });
...
if ($("#volta-fleet-size-range").length) {
  function trees() {
    ...
    $("#volta-calculation-result").text(
      numberWithCommas(((vehicle_number * vehicle_value) / 58).toFixed(2))
    );
  }
  ...
  init: function () {
    this.map = new google.maps.Map(document.getElementById("map"), {
      ...
      styles: [
        ...
        { featureType: "water", stylers: [{ lightness: 80 }] },
        {
          featureType: "water",
          elementType: "geometry",
          stylers: [{ color: "#c9c9c9" }, { lightness: 55 }],
        },
        {
          featureType: "water",
...
$(document).ready(function () {
  ...
  if (localStorage.getItem("utm_data")) {
    ...
    if (
      utm_data.utm_medium.indexOf("cpc") != -1 &&
      utm_data.utm_source.indexOf("google") != -1
    ) {
      utm_data.utm_medium = "Google Ads";
      utm_data.utm_source = "google";
    }
    if (utm_data.utm_medium == "cpc" && utm_data.utm_source == "google")
      utm_data.utm_medium = "Google Ads";
    // Trying to fix a bug [25/11/19] that sends us the full url as source
    if (
      utm_data.utm_source.indexOf("utm_medium=cpc") != -1 &&
      utm_data.utm_source.indexOf("utm_source=google") != -1
    ) {
      utm_data.utm_source = "google";
      utm_data.utm_medium = "Google Ads";
    }
  ...
  $("form .conditional-trigger").on("change", function (e) {
    ...
    direction == "up"
      ? $(selection).slideUp(function () {
          console.log($(":input:hidden", "form .collapse"));
          $(":input:hidden", "form .collapse").prop("required", false);
        })
      : $(selection).slideDown(function () {
          $(":input:visible", "form .collapse").prop("required", true);
        });
  });

  $(".select-wrapper.multiple select").on(
  ...
  if (pageref == "summit") {
    if (window.location.hash) {
      ...
      if (hash == "apply") {
        ...
        $("#summitModal")
          .find(".modal-body #subject-field")
          .val("Apply to Attend WMS 19");
      }
    }
  }
  ...
  if (document.URL.indexOf("/blog/") != -1) {
    ...
    localStorage.setItem(
      "lastBlogTime",
      new Date().toLocaleString("en-GB", { timeZone: "UTC" }).replace(",", "")
    );
  }
  ...
  var hpScrollerWidth =
    $(".featured-quotes__single-quote:nth-child(2)").width() * 3 +
    $(window).width() * 0.2;
  var halfQuoteWidth =
    $(".featured-quotes__single-quote:nth-child(2)").width() / 2;
  $(".home-quotes .mob-scroll").scrollLeft(
    hpScrollerWidth * 0.5 - halfQuoteWidth
  );
